  ...
  return (
    <div className={`flex items-center justify-between rounded-lg border p-3 ${styles.bg} ${styles.border}`}>
      <div className="flex items-center gap-3">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-background">{styles.icon}</div>
        <div>
      ...
      <div className="flex items-center gap-2">
        {item.count !== undefined && <Badge variant={styles.badge}>{item.count}</Badge>}
        {item.linkTo && (
...
const AdminDashboard: React.FC = () => {
  ...
  // Top-up Stats
  const todayTopUps = transactions.filter(
    (t) => t.type === "top_up" && t.createdAt.startsWith(today) && t.status === "completed",
  );
  const todayTopUpCount = todayTopUps.length;
  const todayTopUpAmount = todayTopUps.reduce((sum, t) => sum + t.amount, 0);

  const thisMonthTopUps = transactions.filter(
    (t) => t.type === "top_up" && t.createdAt.startsWith(thisMonth) && t.status === "completed",
  );
  ...
  const trendChartData = useMemo(() => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const dayTopUps = transactions.filter(
        (t) => t.type === "top_up" && t.status === "completed" && t.createdAt.startsWith(dateStr),
      );
                  ...
                  return (
                    <div key={txn.id} className="flex items-center justify-between border-b pb-2 last:border-0">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <Badge
                            variant={txn.type === "top_up" ? "success" : txn.type === "charge" ? "warning" : "info"}
                            className="text-xs"
                          >
                            {txn.type.replace("_", " ")}
                          </Badge>
